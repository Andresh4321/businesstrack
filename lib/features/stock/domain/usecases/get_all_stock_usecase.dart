...
final getAllStockUsecaseProvider = Provider<GetAllStockUsecase>((ref) {
  final stockRepository = ref.read(stockRepositoryProvider);
  return GetAllStockUsecase(stockRepository: stockRepository);
});

class GetAllStockUsecase implements UsecasewithoutParams<List<StockEntity>> {
  final IStockRepository _stockRepository;

  GetAllStockUsecase({required IStockRepository stockRepository})
